...
def value_iteration(problem):
    ...
    for i in range(m):
        for j in range(n):
            if grid[i][j] == "#":
                current_value_pi[i][j] = "#"# type: ignore
    return_value = show_value_pi(current_value_pi, current_idx, m, n)# type: ignore
    current_idx += 1
        ...
        for i in range(m):
            for j in range(n):
                if not check_direction_feasibility(m, n, i, j, grid):
                    current_value_pi[i][j] = "#"# type: ignore
                    policy[i][j] = "#"# type: ignore
                elif grid[i][j] not in "#_S":
                    current_value_pi[i][j] = num(grid[i][j])# type: ignore
                    policy[i][j] = "x"# type: ignore
                else:
                    max_ = -float('inf')
                    max_direction = None
                    ...
                    policy[i][j] = max_direction# type: ignore
                    current_value_pi[i][j] = max_# type: ignore
        return_value += generate_result(current_value_pi, policy, current_idx, m, n)# type: ignore
        current_idx += 1
...
def show_value_pi(value_pi: float, k: int, m: int, n: int) -> str:
    ...
    for i in range(m):
        temp_res = ""
        for j in range(n):
            if type(value_pi[i][j]) is not str:# type: ignore
                temp_res += '|{:7.2f}|'.format(value_pi[i][j])# type: ignore
            else:
                temp_res += "| ##### |"
        temp_res += "\n"
        res += temp_res
