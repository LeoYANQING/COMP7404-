...
"""
Critical Balance of Exploration vs. Exploitation
The decay rate of ε-greedy exploration significantly impacts convergence. Exponential decay (e.g., starting at ε=1.0 with a 0.99 decay factor per step) proved effective. Fast decay risks insufficient exploration, while slow decay delays convergence. Optimal exploration ensures the agent discovers paths avoiding high-penalty states.

    Learning Rate Decay Enhances Stability
Adaptive learning rate decay (e.g., α = 1/𝑛 for state-action visit count 𝑛) stabilized Q-value updates. Global decay (e.g., α=1/√𝑛) caused under-updating in rarely visited states, whereas state-action-specific decay ensured smoother convergence.

    Robustness to Environmental Noise
Despite 80% action success and 10% side-slip probabilities, Q-learning inherently averaged over stochastic transitions. The algorithm achieved ~90% success in finding the optimal policy, demonstrating resilience to noise.

    Handling Multiple Optimal Policies
The environment allowed multiple optimal policies (e.g., different paths with identical cumulative rewards). Q-learning reliably converged to any valid optimal policy, reflected in consistent success rates across runs.

    Heuristic Stopping Criteria
Terminating iterations when Q-value changes fell below a threshold (e.g., max ΔQ < 1e-5 for 100 steps) or policies stabilized (e.g., no changes for 50 steps) ensured termination without referencing ground-truth policies.
"""
...
class Agent:
    def __init__(self, discount, noise, living_reward, grid, alpha):
        ...
        self.start_state_row, self.start_state_col = self.find_start_state()# type: ignore
        self.alpha = alpha
